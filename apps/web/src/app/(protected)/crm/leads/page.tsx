...
export default function LeadsPage() {
  ...
  const columns = [
    ...
    { 
      key: 'value', 
      label: 'Value',
      render: (item: any) => item.value ? `$${item.value.toLocaleString()}` : '-'
    },
    { 
      key: 'status', 
      label: 'Status',
      render: (item: any) => (
        <select 
          ...
          className="text-xs rounded-full px-2 py-1 border-0 bg-indigo-50 text-indigo-700 font-medium focus:ring-2 focus:ring-indigo-500"
        >
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage your potential customers</p>
        </div>
        <button
      ...
      {/* Filters */}
      <div className="flex gap-4 bg-white/50 backdrop-blur-xl p-4 rounded-2xl border border-white/20">
        <div className="relative flex-1 max-w-xs">
          ...
          <select
            ...
            className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg leading-5 bg-white/50 placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          >
